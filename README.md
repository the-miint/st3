...
The C header `st3.h` is generated by the build (via `cbindgen`) into the C ABI
crate's `OUT_DIR`; it is not committed. Copy the freshest generated header to a
stable path with:
...
Then compile and link a C program against it. Point `-I` at the header's
directory and `-L` at the built library, link `-lst3` (and `-lm`):
